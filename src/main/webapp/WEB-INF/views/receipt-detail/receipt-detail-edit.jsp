    ...
    <div class="row">
    	<div class="col-md-12 col-sm-12">
    		<div class="x_panel">
    			...
    			<div class="x_content">
    				<br>
    				<form:form modelAttribute="modelForm" method="POST" servletRelativeAction="/receipt-detail/edit"  cssClass="form-horizontal form-label-left">
    					<form:hidden path="id"/> 					
    					<form:hidden path="active"/>    					
    					<form:hidden path="created"/>
    					
    					<div class="item form-group">
							<label class="col-form-label col-md-3 col-sm-3 label-align" for="receiptCode">Receipt <span class="required">*</span>
							</label>
							<div class="col-md-6 col-sm-6 col-xs-12"> 
								<c:choose>
									<c:when test="${!viewOnly }">
										<form:errors path="receiptCode" cssClass="help-block" cssStyle="color: #dc3545;"></form:errors>
										<form:select path="receiptCode" cssClass="form-control ">
											<form:options items="${mapReceipt }" />
										</form:select>
									</c:when>
									<c:otherwise>
										<form:input path="receipt.code" cssClass="form-control" disabled="true" />
									</c:otherwise>
								</c:choose>
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(
		function () {
			$('#receipt-detaillistId').addClass('cuurent-page').siblings().removeClass('current-page');
			var parent = $('#receipt-detaillistId').parents('li');
			parent.addClass('active').siblings().removeClass('active');
			$('#receipt-detaillistId').parents().show();
		}		
	);
	
	function cancel() {
		window.location.href = '<c:url value="/receipt-detail/list"/>'
	}
</script>